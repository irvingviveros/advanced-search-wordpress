...
class Guaven_woo_search_backend
{
    ...
    public function backend_search_filter($where = '')
    {
        ...
        $recalc_where=apply_filters('gws_sql_restructure_where_part_v2',false,$where,$this->where);
    ...
    public function post_saved_hooks($post_id, $post, $is_update){
        $this->purengine_post_saved($post_id, $post, $is_update);
        $this->trace_for_cron_rebuilder($post_id, $post, $is_update);
    }

    public function trace_for_cron_rebuilder($post_id, $post, $is_update){
        if(($post->post_type != 'product' && $post->post_type != 'product_variation') 
                || ! ($product = wc_get_product($post))) 
            return;
        set_transient('gws_cron_rebuilder_should_run',time(),3600);
    }
