...
where meta_key='_sku' and b.post_type='product_variation' and b.post_parent=" . intval($value->ID));
    ...
    public function enqueue()
    {
        ...
        wp_enqueue_script('guaven_woos_chartist', plugin_dir_url(__FILE__) . 'assets/chartist.min.js?v='.$vers);
    }
    ...
    public function synonym_list($add_synonyms_too, $ptitle)
    {
        ...
        foreach ($synonym_list as $syn) {
            ...
            if 
            (
                in_array(trim($syn_lr[0]), $title_elements) or
                (
                    strpos($ptitle, ' ') !== false and 
                    strpos((html_entity_decode($ptitle)), trim( $syn_lr[0])) !== false
                )
            ) 
            {
            ...
            elseif 
            (
                in_array(trim($syn_lr[1]), $title_elements) or
                (
                    strpos($ptitle, ' ') !== false and 
                    strpos((html_entity_decode($ptitle)), trim($syn_lr[1])) !== false
                )
            ) 
            {
            ...
            elseif 
            (
                in_array(trim(str_replace("_", "-", $syn_lr[0])), $title_elements) or
                (
                    strpos($ptitle, ' ') !== false and 
                    strpos((html_entity_decode($ptitle)), trim(str_replace("_", "-", $syn_lr[0]))) !== false
                )
            ) 
            {
            ...
            elseif 
            (
                in_array(trim(str_replace("_", "-", $syn_lr[1])), $title_elements) or
                (
                    strpos($ptitle, ' ') !== false and 
                    strpos((html_entity_decode($ptitle)), trim(str_replace("_", "-", $syn_lr[1]))) !== false
                )
            ) 
            {
